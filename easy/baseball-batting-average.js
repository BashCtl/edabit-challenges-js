...
The given array represents a season of games. Each array item indicates 
a player's [hits, official at bats] per game. Return a string with 
the player's seasonal batting average rounded to the nearest thousandth.
...
The array includes official at-bats only. No other plate-appearances 
(walks, hit-by-pitches, sacrifices, etc.) are included in the array.
HINT: Think in terms of total hits and total at-bats.

*/

function battingAvg(arr) {
  return (arr.reduce((a, c) => a + c[0], 0) / arr.reduce((a, c) => a + c[1], 0)).toFixed(3).slice(1);
}
