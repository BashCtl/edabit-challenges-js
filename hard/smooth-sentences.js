...
to the first letter the following word (and not case sensitive, so "A" would be the same as "a").
...
Create a function that determines whether the input sentence 
is a smooth sentence, returning a boolean value true if it is, false if it is not.
...
function isSmooth(sentence) {
  ...
  for (let i = 0; i < words.length - 1; i++) {
    if (!words[i].endsWith(words[i + 1].charAt(0))) {
      return false;
    }
  }
  return true;
