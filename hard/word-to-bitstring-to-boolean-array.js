...
Create a function that converts a word to a bitstring and 
then to a boolean array based on the following criteria:
...
function toBoolArray(word) {
  return word
    .split("")
    .map((w) => ((w.charCodeAt() - "a".charCodeAt() + 1) % 2 === 0 ? 0 : 1))
    .map((n) => n === 1);
}
